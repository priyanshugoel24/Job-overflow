...
var appDevJobs = [];
const url = `http://api.adzuna.com/v1/api/jobs/in/search/1?app_id=${process.env.ADZUNA_ID}&app_key=${process.env.ADZUNA_KEY}&results_per_page=20&what=app%20developer&content-type=application/json`;

async function fetchJobs(){
    const res = await fetch(url);
    appDevJobs = await res.json();
    console.log(appDevJobs);
    
}
fetchJobs();


const AppDeveloperJobs = () => {
    return (
        <div className="jobs-list">
        {appDevJobs.results.map((job) => (
...
export default AppDeveloperJobs
